...
const (
	...
	`
	GetPostLikes = `
		SELECT users.* FROM liked_posts
		INNER JOIN users
		ON liked_posts.user_id = users.id
		WHERE liked_posts.post_id = ?
	`
	GetUserLikes = `
		SELECT posts.* FROM liked_posts
		INNER JOIN posts ON liked_posts.post_id = posts.id
		WHERE liked_posts.user_id = ?
		ORDER BY id DESC
	`
	GetPostDislikes = `
		SELECT users.* FROM disliked_posts
		INNER JOIN users ON disliked_posts.user_id = users.id
		WHERE disliked_posts.post_id = ?
	`
	GetUserDislikes = `
		SELECT posts.* FROM disliked_posts
		INNER JOIN posts ON disliked_posts.post_id = posts.id
		WHERE disliked_posts.user_id = ?
		ORDER BY id DESC
	`
	GetSessionUser = `
